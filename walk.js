...
function create(color, dx, dy) {
  const newBall = Object.create(this);
    ...
    // Creates variables for color, dx, dy
    // Creates variables for width & height and set initial values
    // because they are not passed as parameters
    newBall.dx = dx;
    newBall.dy = dy;
    newBall.width = 40;
    newBall.height = 40;
  
    // Sets the newBall.element property and initialize it to a Html element "div"
...
function update(ball, x, y) {
  // The moveTo() function to move the ball
  // The Javascript setTimeout() method to call changeDirectionIfNecessary() and update() every 16ms
    moveTo(ball, x, y);
    setTimeout(function () {
